...
#include "utils.h"
#define PI 3.1415
void saveIntegersToFile(int num1, int num2, int count,int number, double pid_result) {
    ...
    // Write the coordinates and count to the file
    file  <<"The "<<number<<" photo, "<< count << "times: (" << num1 << ", " << num2 << ")" << "  "<<" pid_result: "<<pid_result<<std::endl;

    // Close the file
    file.close();

    std::cout << "Numbers have been saved to output.txt." << std::endl;
}
...
double PID(double x, double y)
{
    int turn_flag = 0;
    double pid_error = 0;
    int center = 320;
    double pid_integral = 0, pid_derivative = 0, pid_prev_error = 0, pid_result = 0;
    double Kp = 3, Ki = 0.002, Kd = 1;
    int Kp_1000 = 1001, Ki_1000 = 250, Kd_1000 = 6;

        pid_integral += pid_error;

        pid_derivative = (pid_error - pid_prev_error) * 10;

        pid_result = pid_error * Kp + pid_integral * Ki + pid_derivative * Kd;

        pid_prev_error = pid_error;

        return pid_result;
}
double draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch, int number) {
  ...
  for (size_t i = 0; i < img_batch.size(); i++) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      count++;
      //Calculating Angle(cm)
      int f = 4;
      int height = 11;
      //std::string class_id ="arrow";
      pid_result += PID(x,y);
      target++;
      saveIntegersToFile(x, y, count, number, pid_result);
      cv::circle(img,cv::Point(x,y),10,cv::Scalar(0, 201, 87), -1,8,0);
      cv::line(img,cv::Point(r.x,r.y),cv::Point(r.x+r.width,r.y+r.height),cv::Scalar(0, 0, 255),3,8,0);
      cv::line(img,cv::Point(r.x+r.width,r.y),cv::Point(r.x,r.y+r.height),cv::Scalar(0, 0, 255),3,8,0);
      cv::rectangle(img, r, cv::Scalar(0x27, 0xC1, 0x36), 2);
      //cv::putText(img, class_id, cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
      cv::putText(img, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
      class_id=(int)res[j].class_id;
    }
    if(class_id==0)
    return pid_result/target;
    else return 0;
